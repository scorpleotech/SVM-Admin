...
const AddDemo = () => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/testdrive`,
        updatedValues
      );
      if (response.status === 201) {
        message.success("Demo Bokking Added Successfully");
        Router.push("/demo/list");
      }
    } catch (error) {
      message.error("Demo Bokking Failed " + error.response.data.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <Card className="card" title={"Demo Booking Add"}>
        <Form
          name="addDemoForm"
          onFinish={onFinish}
        ...
        >
          ...
          {selectedModel && (
            <Form.Item label="Image">
              <Image
                src={models.find((model) => model._id === selectedModel)?.image}
                alt="Model Image"
                style={{ maxWidth: 200 }}
              />
            </Form.Item>
