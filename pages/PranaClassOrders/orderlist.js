...
const PranaClassOrders = ({ getData = [] }) => {
  const intl = useIntl();
  // Sample data for prana class orders - filtered for Pending and Booked status only
  const [allData, setAllData] = useState([
    {
      _id: "1",
      reservation_number: "PCO-2025-001",
    ...
    {
      _id: "2",
      reservation_number: "PCO-2025-002",
    ...
    {
      _id: "4",
      reservation_number: "PCO-2025-004",
  ...
  const getDataFc = () => {
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/api/pranaclass`)
      .then((res) => {
        console.log("PranaClass data", res.data);
      ...
      .catch((err) => console.log("PranaClass API Error:", err));
  };
  ...
  const getDataStatusFc = (target = "All") => {
    ...
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/api/pranaclass/status/` + target)
      .then((res) => {
        if (res.data.data && res.data.data.length > 0) {
          const rawData = res.data.data;
          // Map the data to match table structure
      ...
      .catch((err) => console.log("PranaClass Status API Error:", err));
  };
  ...
  const deleteData = (id, imagePath = 0) => {
    axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/pranaclass/${id}`).then(() => {
      message.success("PranaClass Order Deleted Successfully");
      getDataFc();
      Router.push("/PranaClassOrders/orderlist");
    });
  };
  ...
  return (
    <div>
      {role["ordersview"] ? (
        <>
          <h5 className="mr-5 ">
            Prana Class Orders List  
          </h5>
          
          <Link
            href={`${process.env.NEXT_PUBLIC_API_URL}/api/pranaclass/export/order`}
            target="_blank"
            legacyBehavior
            className="mr-2"
          >
...
PranaClassOrders.getInitialProps = async ({ req }) => {
  if (!req?.headers?.cookie) {
    return {};
  } else {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/pranaclass`, {
      ...
      console.error("PranaClass getInitialProps error:", error);
      return { getData: [] };
    }
  }
};

export default PranaClassOrders;