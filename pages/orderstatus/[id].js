...
const Default = ({ getData = [] }) => {
  ...
  function getDataFc() {
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/orderstatus/${id}`)
      .then((response) => {
  ...
  const onSubmit = async (Data) => {
    if (Data.image != undefined && state.image != Data.image) {
      axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/upload/deleteorderstatusimage`,
        {
          path: state.image,
        }
      );
      ...
      const dataImage = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/upload/uploadorderstatusimage`,
        formData,
    ...
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/orderstatus/${id}`, Data)
      .then((res) => {
...
Default.getInitialProps = async ({ req, query }) => {
  if (!req?.headers?.cookie) {
    ...
    const getData = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/orderstatus/` + query.id,
      {
