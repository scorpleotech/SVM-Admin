...
const Default = ({ dataCityOption = [], dataCity = [], getData = [] }) => {
  ...
  function getDataFc() {
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/customers/${id}`)
      .then((response) => {
  ...
  const getCountry = () => {
    axios.get(`${process.env.NEXT_PUBLIC_API_URL}/country`).then((getData) => {
  ...
  const getCity = () => {
    axios.get(`${process.env.NEXT_PUBLIC_API_URL}/turkey`).then((getData) => {
      const dataManipulate = [];
      for (const i in getData.data) {
        dataManipulate.push({
  ...
  function updatePassword(pass) {
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/staff/updatePasswordCustomer`, {
        _id: id,
        password: pass,
      })
      .then((res) => {
  ...
  const onSubmit = (Data) => {
    ...
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/customers/${id}`, Data)
      .then((res) => {
        ...
        <Row>
          <Col md={24}>
            <Card className="card" title="Customer Add">
              ...
              >
                <Input name="email" onChange={onChangeNameValue} />
              </Form.Item>
              <Form.Item
                name="password"
...
Default.getInitialProps = async ({ req, query }) => {
  if (!req?.headers?.cookie) {
    ...
    const getData = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/customers/` + query.id,
      {
    ...
    const getDataCity = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/turkey`,
      {
        headers: req ? { cookie: req.headers.cookie } : undefined,
      }
    );
