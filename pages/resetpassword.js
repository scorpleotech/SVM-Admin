...
const SignInPage = () => {
  ...
  const getUserWithToken = async (token) => {
    await axios
      .get(
        `${process.env.NEXT_PUBLIC_API_URL}/users/reset?resetPasswordToken=` +
          token
      )
      .then((res) => {
  ...
  const onSubmit = (Data) => {
    axios
      .put(`${process.env.NEXT_PUBLIC_API_URL}/users/updatePasswordViaEmail`, {
        username: username,
    ...
    <>
      <Row gutter={[16, 16]}>
        <Col sm={6} offset={3} xs={18}>
          ...
          <div className="text-center fs-10 mb-5">Reset Password</div>
          {username ? (
            <Form
