...
const Default = ({ getData = [] }) => {
  ...
  const deleteData = (id, imagePath = 0) => {
    axios
      .delete(`${process.env.NEXT_PUBLIC_API_URL}/cargoes/${id}`)
      .then(() => {
  ...
  return (
    <div>
      {role["cargoes/add"] ? (
        <Link href={"/cargoes/add"} legacyBehavior>
          <Button
            type="primary"
...
Default.getInitialProps = async ({ req }) => {
  if (!req?.headers?.cookie) {
    ...
    const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/cargoes`, {
