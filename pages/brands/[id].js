...
const Default = ({ getData = [] }) => {
  ...
  function getDataFc() {
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/brands/${id}`)
      .then((response) => {
  ...
  const onSubmit = async (Data) => {
    if (Data.image != undefined && state.image != Data.image) {
      axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/upload/deletebrandsimage`,
        {
          path: state.image,
        }
      );
      ...
      const dataImage = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/upload/uploadbrandsimage`,
        formData,
    ...
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/brands/${id}`, Data)
      .then((res) => {
...
Default.getInitialProps = async ({ req, query }) => {
  if (!req?.headers?.cookie) {
    ...
    const getData = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/brands/` + query.id,
      {
