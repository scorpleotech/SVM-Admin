...
const Default = ({ getData = [] }) => {
  ...
  const activeOrDeactive = (id, deg) => {
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/banner/active/${id}`, {
        isActive: !deg,
      })
      .then(() => {
        message.success(`Banner ${!deg ? "Activated" : "Deactivated"} `);
        getDataFc();
  ...
  const deleteData = (id, imagePath = 0) => {
    axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/banner/${id}`).then(() => {
    ...
    if (imagePath != 0) {
      axios
        .post(`${process.env.NEXT_PUBLIC_API_URL}/upload/deletebannerimage`, {
          path: imagePath,
        })
        .then(() => {
  ...
  return (
    <div>
      {role["banner/add"] ? (
        <Link href={"/banner/add"} legacyBehavior>
          <Button
            type="primary"
