...
const Default = ({ getData = [] }) => {
  ...
  const getDataFc = () => {
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/aboutus`)
      .then((res) => {
  ...
  const activeOrDeactive = (id, deg) => {
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/aboutus/active/${id}`, {
        isActive: !deg,
      })
      .then(() => {
        message.success(`Aboutus ${!deg ? "Activated" : "Deactivated"} `);
        getDataFc();
  ...
  const deleteData = (id, imagePath = 0) => {
    axios
      .delete(`${process.env.NEXT_PUBLIC_API_URL}/aboutus/${id}`)
      .then(() => {
    ...
    if (imagePath != 0) {
      axios
        .post(`${process.env.NEXT_PUBLIC_API_URL}/upload/deleteaboutusimage`, {
          path: imagePath,
        })
        .then(() => {
          message.success("Aboutus Deleted Successfully");
  ...
  return (
    <div>
      {role["aboutus/add"] ? (
        <Link href={"/aboutus/add"} legacyBehavior>
          <Button
            type="primary"
