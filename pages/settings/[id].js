...
const Default = () => {
  ...
  function getDataFc() {
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/settings/${id}`)
      .then((response) => {
  ...
  const onSubmit = async (Data) => {
    if (Data.image != undefined && state.image != Data.image) {
      axios.post(`${process.env.NEXT_PUBLIC_API_URL}/upload/deletelogoimage`, {
        path: state.image,
      });
      ...
      const dataImage = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/upload/uploadlogoimage`,
        formData,
    ...
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/settings/${id}`, Data)
      .then((res) => {
