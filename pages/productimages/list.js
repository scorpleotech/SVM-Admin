...
const Default = ({ getData = [] }) => {
  ...
  const getDataFc = (id) => {
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/productimages`)
      .then((res) => {
  ...
  const deleteData = (id, imagePath = 0, quertId) => {
    ...
    axios
      .delete(`${process.env.NEXT_PUBLIC_API_URL}/productimages/${id}`)
      .then(() => {
    ...
    <div>
      ...
      <div className="w-full">
        {role["productimages/add"] ? (
          <Link href={"/productimages/add?id=" + id} legacyBehavior>
            <Button
              type="primary"
...
Default.getInitialProps = async ({ req, query }) => {
  if (!req?.headers?.cookie) {
    ...
    const res = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/productimages`,
      {
