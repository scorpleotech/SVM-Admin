...
const Default = ({ getData = [] }) => {
  ...
  async function getDataFc() {
    await axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/event/${id}`)
      .then((response) => {
  ...
  const onSubmit = async (Data) => {
    ...
    if (Data.image != undefined && state.image != Data.image) {
      axios.post(`${process.env.NEXT_PUBLIC_API_URL}/upload/deleteeventimage`, {
        path: state.image,
      });
      ...
      const dataImage = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/upload/uploadeventimage`,
        formData,
    ...
    console.log("the data", Data);
    axios
      .put(`${process.env.NEXT_PUBLIC_API_URL}/event/${id}`, Data)
      .then((res) => {
  ...
  return (
    <div>
      ...
      <Form
        ...
        scrollToFirstError
      >
        <Card className="card" title={"Event Edit"}>
          <Form.Item
            name="title"
...
Default.getInitialProps = async ({ req, query }) => {
  if (!req?.headers?.cookie) {
    ...
    const getData = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/event/` + query.id,
      {
