...
const Default = ({ getData = [], getCategories = [] }) => {
  ...
  function getDataFc() {
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/homeslider/${id}`)
      .then((response) => {
  ...
  const getDataCategory = () => {
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/homeslider`)
      .then((res) => {
  ...
  const onSubmit = async (Data) => {
    if (Data.image != undefined && state.image != Data.image) {
      axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/upload/deletehomesliderimage`,
        {
          path: state.image,
        }
      );
      ...
      const dataImage = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/upload/uploadhomesliderimage`,
        formData,
    ...
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/homeslider/${id}`, Data)
      .then((res) => {
...
Default.getInitialProps = async ({ req, query }) => {
  if (!req?.headers?.cookie) {
    ...
    const getDataCategories = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/homeslider`,
      {
    ...
    const getData = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/homeslider/` + query.id,
      {
        headers: req ? { cookie: req.headers.cookie } : undefined,
      }
    );
