...
const Default = ({ getCategories = [] }) => {
  ...
  const onSubmit = async (Data) => {
    ...
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/blog`, Data)
      .then((res) => {
        if (res.data.variant == "error") {
          message.error(
            intl.messages["app.pages.blog.notAdded"] + res.data.messagge
          );
        } else {
          message.success(intl.messages["app.pages.blog.Added"]);
  ...
  return (
    <div>
      ...
      <Form
        ...
        scrollToFirstError
      >
        <Card className="card" title={"Blog Add"}>
          <Form.Item
            name="title"
