...
const Default = ({ getData = [] }) => {
    ...
    // },
    {
      ...
      render: (text, record) => (
        ...
        {console.log("record =",record)}
          {record?.url_type == "youtube" ? (
           record?.file_url
    //         <div style={{width:"175px",height:"50px"}} >
    //           <div
    //   dangerouslySetInnerHTML={{ __html: record?.file_url?.replace('width="560"', 'width="100%"').replace('height="315"', 'height="100%"')}}
    // />
    //          </div>
          ) : (
            
            <Image
              src={`${process.env.NEXT_PUBLIC_API_URL}${record?.file_url?.replace("../admin/public/","")}`}
              height={80}
            />
          )}
        </>
  ...
  return (
    <div>
      {role["gallery/add"] ? (
        <Link href={"/gallery/add"} legacyBehavior>
          <Button
            type="primary"
