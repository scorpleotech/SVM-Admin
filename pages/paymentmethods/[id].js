...
const Default = ({ getData = [] }) => {
  ...
  function getDataFc() {
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/paymentmethods/${id}`)
      .then((response) => {
  ...
  const onSubmit = async (Data) => {
    if (Data.image != undefined && state.image != Data.image) {
      axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/upload/deletepaymentmethodsimage`,
        {
          path: state.image,
        }
      );
      ...
      const dataImage = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/upload/uploadpaymentmethodsimage`,
        formData,
    ...
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/paymentmethods/${id}`, Data)
      .then((res) => {
...
Default.getInitialProps = async ({ req, query }) => {
  if (!req?.headers?.cookie) {
    ...
    const getData = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/paymentmethods/` + query.id,
      {
