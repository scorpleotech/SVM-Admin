...
const Default = ({ getData = [] }) => {
  ...
  const activeOrDeactive = (id, deg) => {
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/paymentmethods/active/${id}`, {
        isActive: !deg,
      })
      .then(() => {
  ...
  const deleteData = (id, imagePath = 0) => {
    axios
      .delete(`${process.env.NEXT_PUBLIC_API_URL}/paymentmethods/${id}`)
      .then(() => {
  ...
  return (
    <div>
      {role["paymentmethods/add"] ? (
        <Link href={"/paymentmethods/add"} legacyBehavior>
          <Button
            type="primary"
...
Default.getInitialProps = async ({ req }) => {
  if (!req?.headers?.cookie) {
    ...
    const res = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/paymentmethods`,
      {
