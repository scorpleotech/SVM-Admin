...
const Default = ({ getData = [], getCategories = [] }) => {
  ...
  function getDataFc() {
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/topmenu/${id}`)
      .then((response) => {
  ...
  const onSubmit = (Data) => {
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/topmenu/${id}`, Data)
      .then((res) => {
...
Default.getInitialProps = async ({ req, query }) => {
  if (!req?.headers?.cookie) {
    ...
    const getData = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/topmenu/` + query.id,
      {
    ...
    const getDataCategories = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/topmenu`,
      {
        headers: req ? { cookie: req.headers.cookie } : undefined,
      }
    );
