...
const Default = ({ getData = [] }) => {
  ...
  const getDataFc = () => {
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL}/topmenu`)
      .then((response) => {
  ...
  const activeOrDeactive = (id, deg) => {
    axios
      .post(`${process.env.NEXT_PUBLIC_API_URL}/topmenu/active/${id}`, {
        isActive: !deg,
      })
      .then(() => {
  ...
  const deleteData = (id) => {
    axios
      .delete(`${process.env.NEXT_PUBLIC_API_URL}/topmenu/${id}`)
      .then(() => {
  ...
  return (
    <div>
      {role["topmenu/add"] ? (
        <Link href="/topmenu/add" legacyBehavior>
          <Button
            type="primary"
...
Default.getInitialProps = async ({ req }) => {
  if (!req?.headers?.cookie) {
    ...
    const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/topmenu`, {
